...
export async function scrapePopularAnimes() {
    try {
        const { data } = await axios.get('https://gogoanime3.cc/');
...
export async function searchAnime(query) {
    try {
        const { data } = await axios.get(`https://gogoanime3.cc/search.html?keyword=${encodeURIComponent(query)}`);
        const $ = cheerio.load(data);
...
export async function scrapeAnimeDetails(url) {
    try {
        ...
        for (let episodeNumber = 1; episodeNumber <= totalEpisodes; episodeNumber++) {
            ...
            const episodeLink = `https://gogoanime3.cc/${formattedTitle}-episode-${episodeNumber}`; // Adjusted format
...
export async function scrapePopularAiringAnimes() {
    try {
        const { data } = await axios.get('https://gogoanime3.cc/popular.html'); // Change the URL to the popular anime page
        const $ = cheerio.load(data);
...
export async function scrapeNewAnimeThisSeason() {
    try {
        const { data } = await axios.get('https://gogoanime3.cc/new-season.html');
        const $ = cheerio.load(data);
        ...
        $('.last_episodes .items li').each((index, element) => {
            ...
            const link = `https://gogoanime3.cc${$(element).find('.name a').attr('href')}`;

            if (title && imageUrl) {
                const urlSlug = createAnimeUrlSlug(title);
                newAnimes.push({ title, imageUrl, link, urlSlug });
            }
        });

        console.log('Scraped animes:', newAnimes); // Log the scraped data here
